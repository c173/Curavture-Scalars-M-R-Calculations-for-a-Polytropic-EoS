...
G = 1
c = 1
G_convo = 6.67430e-8  # Gravitational constant cm^3/(g*s)^
c_convo = 2.998e+10  # cm/s
k = 8 * pi * G / (c ** 4)  # cm^7/(g*s^6)
m_sun = 1.989e33 * G_convo / c_convo ** 2
# -----------------------------------------------
...
density, pressure = np.loadtxt('./data/AP4_cgs.dat', float, usecols=(0, 1), unpack=True)

# density = conv_rho * density
# pressure = conv_P * pressure

density = density * G_convo / c_convo ** 2
pressure = pressure * G_convo / c_convo ** 4
...
def der_P(r, rho, m, P, lm):
    P_t = P + (lm / 3) * (rho + 3 * P) * (rho + P) * r ** 2 / (1 - 2 * m / r)
    term1 = -(rho + P)
    term2 = (m + 4 * pi * r ** 3 * P)
    term3 = r * (r - 2 * m)
    term4 = (2 / r) * (P_t - P)
...
if flag == 0:
    f = open("radial8.out", 'a')  # append the data to the file # please delete the file before new start
    g = open("curvatures8.out", 'a')  # append the data to the file # please delete the file before new start
    rho_c = rho_max
...
def radial(rho_c, lm):
    ...
    while P > P_min:
        ...
        p_k1 = der_P(r, rho, m, P, lm) * dr
        m_k1 = der_m(r, rho) * dr

        phi_k1 = der_phi(der_P(r, rho, m, P, lm), rho, P) * dr
        ...
        rho1 = 10 ** rho_interp(np.log10(P1))
        p_k2 = der_P(r1, rho1, m1, P1, lm) * dr
        m_k2 = der_m(r1, rho1) * dr

        phi_k2 = der_phi(der_P(r1, rho1, m1, P1, lm), rho1, P1) * dr
        ...
        rho2 = 10 ** rho_interp(np.log10(P2))
        p_k3 = der_P(r2, rho2, m2, P2, lm) * dr
        m_k3 = der_m(r2, rho2) * dr

        phi_k3 = der_phi(der_P(r2, rho2, m2, P2, lm), rho2, P2) * dr
        ...
        rho3 = 10 ** rho_interp(np.log10(P3))
        p_k4 = der_P(r3, rho3, m3, P3, lm) * dr
        m_k4 = der_m(r3, rho3) * dr

        phi_k4 = der_phi(der_P(r3, rho3, m3, P3, lm), rho3, P3) * dr
        ...
        H_m = m / der_m(r, rho)
        H_p = P / np.abs(der_P(r, rho, m, P, lm))
        H = (H_m * H_p) / (H_m + H_p)
...
if flag == 0:
    ...
    fully_contracted_Ricci, Kretschmann_scalar = radial(rho_c, 2)
    f.close()
    g.close()
...
if flag == 1:
    ...
    lm_distribution = np.linspace(-2, 2, 9)
    count = 0
    m = 0
    for lm in lm_distribution:
        f = open("MR" + str(m) + ".out", 'a')
        m += 1
        for rho_c in rho_central:
            Mass, Radius, stiffness, compactness, P_c, omega, j, moi, Ricci_scalar, Weyl_scalar, \
            fully_contracted_Ricci, Kretschmann_scalar = radial(rho_c, lm)
            print(count, lm, rho_c / 1E15, P_c / 1e35, Mass, Radius)
            dat = [rho_c, Mass, Radius, stiffness, compactness, moi * c_convo ** 2 / G_convo, Kretschmann_scalar]
